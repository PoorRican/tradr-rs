...
impl Persistence for Portfolio {
    fn save(&mut self, path: &Path) -> Result<(), Error> {
        ...
        CsvWriter::new(file)
            .include_header(true)
            .finish(&mut self.executed_trades)
            .unwrap();
        ...
        CsvWriter::new(file)
            .include_header(true)
            .finish(&mut self.capital_ts.clone().into())
            .unwrap();
        ...
        CsvWriter::new(file)
            .include_header(true)
            .finish(&mut self.assets_ts.clone().into())
            .unwrap();
    ...
    fn load(path: &Path) -> Result<Self, Error> {
        ...
        for result in rdr.records() {
            let record = result?;
            let point = NaiveDateTime::parse_from_str(&record[0], DATETIME_FORMAT).unwrap();
...
mod tests {
    ...
    fn test_save() {
        ...
        portfolio.add_executed_trade(ExecutedTrade::new_without_cost(
            "test_id".to_string(),
            Side::Buy,
            100.0,
            1.0,
